...
class JBEventItem extends JBEvent
{
    ...
    public static function afterRenderLayout($event)
    {
        ...
        if ($params['layout'] == 'full') {
            ...
            $itemId   = $item->id;
            $oldGroup = $app->jbsession->getGroup('recently_viewed');

            if (isset($oldGroup[$itemId])) {
                unset($oldGroup[$itemId]);
            } else {
                $newGroup[$itemId] = array(
                    'type'  => $item->type,
                    'appId' => $item->application_id,
                );
            }

            $newGroup = $app->jbarray->unshiftAssoc($oldGroup, $itemId, array(
                'type'  => $item->type,
                'appId' => $item->application_id,
            ));

            $app->jbsession->setGroup($newGroup, 'recently_viewed');
            unset($newGroup);
            //$app->jbsession->clearGroup('mod_jbzoo_item');
        }
    }
}