...
class JBMoneyHelper extends AppHelper
{
    ...
    {
        $result    = array();
        $xmlString = $this->_loadByUrl($this->_serviceCBR);
        if (empty($xmlString)) {
            return array();
        }
        ...
        if ($xml = simplexml_load_string($xmlString)) {
            ...
            foreach ($xml as $row) {
                $value   = (float)$this->clearValue((string)$row->Value);
    ...
    {
        $mode = (defined('JBZOO_CONFIG_CURRENCY_ONLINE') ? JBZOO_CONFIG_CURRENCY_ONLINE : 'manual');
        return $mode;
    }
