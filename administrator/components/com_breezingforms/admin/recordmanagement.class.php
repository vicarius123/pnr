...
class bfRecordManagement{
    ...
    function getListRecords(){
        ...
        foreach($elements As $element){
            ...
            if($element['type'] == 'Checkbox' || $element['type'] == 'Checkbox Group' || $element['type'] == 'Select List'){
                 $selectors .= "Trim( Both ', ' From GROUP_CONCAT( DISTINCT ( Case When subrecords.`name` = '{$element['name']}' Then subrecords.`value` Else '' End ) Order By subrecords.`id` SEPARATOR ', ' ) ) As `bfrecord_custom_{$element['name']}` ";
             }else{
        ...
        $db->setQuery(
             "   Select Distinct SQL_CACHE SQL_CALC_FOUND_ROWS "
             . $selectors
