...
abstract class AngieHelperIni
{
	...
	/**
	 * Parse an INI file and return an associative array. This monstrosity is required because some utter morons who
	 * think they are hosts have disabled PHP's parse_ini_string() function for "security reasons". Apparently their
	 * blatant ignorance doesn't allow them to discern between the innocuous parse_ini_string and the potentially
	 * dangerous ini_set, leading them to disable the former and let the latter enabled. In other words, THIS CLASS IS
	 * ONLY HERE TO FIX STUPID.
	 *
	...
	public static function parse_ini_string($data, $process_sections)
	{
		...
		return parse_ini_string($data, $process_sections);
	}
