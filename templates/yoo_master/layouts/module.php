...
if ($style == '') {
	...
	if ($module->position == 'search') $style = 'box';
	if ($module->position == 'footer') $style = 'box';
}
...
// force module style
if (in_array($module->position, array('absolute', 'background', 'breadcrumbs', 'logo', 'banner', 'debug'))) {
	$style = 'raw';
	$showtitle = 0;
}
if (in_array($module->position, array('headerbar', 'toolbar-r' ,'toolbar-l'))) {
	$style = '';
	$showtitle = 0;
}
...
//		$style			.= ($color) ? ' mod-box-'.$color : '';
		...
		if ($suffix == 'parentname')
		{
			$menu_top_item = JFactory::getApplication()->getMenu();
			$parent_top_item = $menu_top_item->getItem( $menu_top_item->getActive()->parent_id );
			$parentname_top_item = $parent_top_item->title;
			if ($parentname_top_item)
			{
				$title_template = '<h3 class="module-title">'.$parentname_top_item.'</h3>';
			}
			else
			{
				$parent_top_item = $menu_top_item->getActive();
				$parentname_top_item = $parent_top_item->title;
				$title_template = '<h3 class="module-title">'.$parentname_top_item.'</h3>';
				echo '<style>';
				echo '@media (max-width: 959px) { #sidebar-a { display: none; } }';
				echo '</style>';
			}
		}
		else
